...
export async function refreshAccessTokenHandler(req: Request, res: Response) {
	...
	if (!refreshToken) return res.status(401).send("Could not refresh access token");

	const decoded = verifyJwt<{ session: string }>(refreshToken as string, "refreshTokenPublicKey");
